...
def generate_other_reports(time: datetime, squad_id):
    inline_keys = [[InlineKeyboardButton('<< ' + (time - timedelta(hours=4)).strftime('dd-mm-yyyy hh:mm'),
                                         callback_data=json.dumps(
                                                     {'t': QueryType.OtherReport.value,
                                                      'ts': (time - timedelta(hours=4)).timestamp(),
                                                      'c': squad_id}))]]
    if time + timedelta(hours=4) < datetime.now():
        inline_keys[0].append(InlineKeyboardButton((time + timedelta(hours=4)).strftime('dd-mm-yyyy hh:mm') + ' >>',
                                                   callback_data=json.dumps(
                                                     {'t': QueryType.OtherReport.value,
                                                      'ts': (time + timedelta(hours=4)).timestamp(),
                                                      'c': squad_id})))
