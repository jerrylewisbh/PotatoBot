...
from core.types import user_allowed, Character, Profession, UserQuest
...
def quest_statistic(bot: Bot, update: Update, session):
    stats = session.query(func.avg(UserQuest.exp)).group_by(UserQuest.level, UserQuest.location.).all()
